        ...
        <main>
          <Section id="basic-page">
            <header className="text-center">
              <Title level="h1" content={ content["faqs-title"] } onSave={this.saveHandler('faqs-title')} />
            </header>
              ...
              map(categories, (category, key) => {
                return(
                  <div key={`category-${key}`}>
                    <Typography variant="display3" color="default">{key}</Typography>
                    { category.map((question, i) => (
                      <ExpansionPanel key={`category-${key}-question-${i}`}>
                        <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
                          <Title level="h4" content={question["question"]} onSave={this.editFaq(i, "question")} />
                        </ExpansionPanelSummary>
                        <ExpansionPanelDetails>
                          <Paragraph content={question["answer"]} onSave={this.editFaq(i, "answer")} />
                        </ExpansionPanelDetails>
                      </ExpansionPanel>
                    ))}
                    {
                      this.props.isEditingPage && <Button color="default" variant="contained" onClick={this.addFaq(key)}>Add question</Button>
                    }
                  </div>
                )
              })
